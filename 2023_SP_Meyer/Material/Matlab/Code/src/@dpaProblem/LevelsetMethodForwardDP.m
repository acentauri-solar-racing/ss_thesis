...
%   level set case and evaluates the optimal state trajectory, cost-to-go
%   and inputs.
...
%
%   [J, X, U, D, dpOutput] = LevelsetMethodForwardDP(obj, dpStateInit)
%
...
%               U               Optimal inputs
%               D               Disturbance
%               dpOutputs       Outputs from model function
%
%   See also dpaProblem/GeneralForwardDP
...
%   28.03.2020  JR  remove interpolation for mixed-inputs (continuous and
%                   discrete) -> reoptimize as soon as one input is discr.
...
function [J, X, U, D, dpOutput] = LevelsetMethodForwardDP(obj, dpStateInit)
...
for k = 1:N-1
    ...
    [J{k}, X{k+1}, U{k}, dpOutput{k}] = levelsetForwardFun(obj, ...
