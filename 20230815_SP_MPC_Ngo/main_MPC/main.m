...
% the indexes are
args.lbx(1:n_states:n_states*(N+1),1) = 13;                     %state velocity lower bound 
...
args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = -1000; % P_brake lower bound
...
% VALUES
%simvar.alpha = linspace(0.00, 0.00, 2*sim_dist/h)';          % continuosly uphill
simvar.alpha = par.Route.incl';                                   % actual inclination 
simvar.G = linspace(300, 300, (par.s_tot+par.N_horizon*par.s_step)/h)';                 % rising sun  
...
while mpciter < max_iter
    ...
    args.p = [x0; vars_update; u_prev(1); SoC_target(N+mpciter); v_prev; t_prev]; % set the values of the parameters vector, set the slope prediction = [x0; road_slope_over_horizon N]
    ...
    diary(['diagnostic/diary_' num2str(mpciter+1)])
...
%% Diagnostics
diagnostics = diagnostic(max_iter, dist);
diagnostics.alwaysFeasible

%%
%2083
final_time = xx(3,end)      % [s] total state.time
average_velocity = sim_dist/final_time
visualize_plot
